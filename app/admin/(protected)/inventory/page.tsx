...
export default async function InventoryOverviewPage() {
  ...
  const all = items || []
  const stats = { total: all.length, low: all.filter(i => i.quantity_in_stock <= i.reorder_threshold).length, transit: all.reduce((s, i) => s + (i.quantity_in_transit || 0), 0), damaged: all.reduce((s, i) => s + (i.quantity_damaged || 0), 0) }
  const cfg: Record<string, { badge: string; sign: string }> = { add: { badge: "bg-emerald-50 text-emerald-700", sign: "+" }, sold: { badge: "bg-rose-50 text-rose-700", sign: "-" }, damaged: { badge: "bg-amber-50 text-amber-700", sign: "-" }, transit: { badge: "bg-sky-50 text-sky-700", sign: "+" } }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Link href="/admin/inventory/scan" className="flex items-center justify-between rounded-xl border-2 border-[#0B7C79] bg-[#0B7C79] p-5 text-white shadow-sm hover:bg-[#0a6b68] transition-colors group">
          <div><ScanLine className="h-6 w-6 mb-2" /><p className="font-bold">Scan Session</p><p className="text-sm text-white/70">Scan multiple products</p></div>
        ...
        <Link href="/admin/inventory/products" className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:border-emerald-300 transition-all group">
          <div><Package className="h-6 w-6 text-gray-600 mb-2" /><p className="font-bold text-gray-900">Products</p><p className="text-sm text-gray-500">Manage and print labels</p></div>
        ...
        <Link href="/admin/inventory/activity" className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:border-emerald-300 transition-all group">
          <div><BarChart3 className="h-6 w-6 text-gray-600 mb-2" /><p className="font-bold text-gray-900">Activity Log</p><p className="text-sm text-gray-500">Full movement history</p></div>
          <ArrowRight className="h-5 w-5 text-gray-400 group-hover:translate-x-1 transition-transform" />
        </Link>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        {(!moves || moves.length === 0) ? (
          <div className="py-10 text-center text-sm text-gray-400">No activity yet — start a scan session!</div>
        ) : (
            ...
            {moves.map((m: any) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <span className={"rounded-full px-2 py-0.5 text-[11px] font-medium " + c.badge}>{m.action}</span>
                  <p className="text-sm font-bold text-gray-900">{c.sign}{m.quantity}</p>

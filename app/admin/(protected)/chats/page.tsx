...
export default function AdminChatsPage() {
  ...
  const toggleSetting = async (field: "enabled" | "visible") => {
    ...
    if (field === "enabled") setChatEnabled(nv)
    else setChatVisible(nv)
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      try {
        const r = await fetch(`/api/chat?conversation_id=${activeId}`)
        const d = await r.json()
  ...
  const resolveChat = async () => {
    ...
    await fetch("/api/admin/chats", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ conversation_id: activeId, action: "resolve" }),
    })
    ...
    <main className="min-h-screen bg-[#F7F5EF]">
      ...
      <div className="mx-auto w-full max-w-screen-2xl px-6 py-6">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-[#1b1b1b]">Customer Chats</h2>
            <p className="mt-1 text-sm text-gray-500">
              Manage live conversations, escalations, and AI-assisted responses.
            </p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="flex h-[calc(100vh-250px)] min-h-[620px] min-w-0 overflow-hidden">
            <aside className="flex h-full w-80 flex-shrink-0 flex-col border-r border-gray-200 bg-white">
                ...
                {loading ? (
                  ...
                  filteredConvs.map((c) => (
                    <div
                      ...
                      className={`flex cursor-pointer items-start gap-3 border-b border-gray-50 px-4 py-4 hover:bg-gray-50 ${
            ...
            <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-gray-50">
              {!activeId ? (
                ...
                <>
                  ...
                  {activeConv?.status !== "resolved" && (suggestion || loadingSuggestion) && (
                    <div className="flex-none border-t border-purple-100 bg-purple-50 px-5 py-3">
                  ...
                  {activeConv?.status !== "resolved" ? (
                    <div className="flex-none border-t border-gray-200 bg-white px-5 py-4">
                      <div className="mx-auto flex max-w-3xl items-end gap-2">
                        ...
                        <button
                          ...
                          className="mb-5 flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-[#0B7C79] text-white hover:bg-[#0a6b68] disabled:opacity-40"
                        >
                          {sending ? (
...
function StatusDot({ status }: { status: string }) {
  if (status === "escalated") {
    return <span className="h-2 w-2 rounded-full bg-amber-500 animate-pulse"></span>
  }
  if (status === "resolved") {

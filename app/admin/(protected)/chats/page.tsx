...
export default function AdminChatsPage() {
  ...
  const loadMessages = useCallback(async (conversationId: string) => {
    ...
    try {
      const r = await fetch(`/api/chat?conversation_id=${conversationId}`, { cache: "no-store" })
      const d = await r.json()
  ...
  const loadSuggestion = useCallback(
    async (conversationId: string, status?: string) => {
      ...
      try {
        const r = await fetch(`/api/admin/chats/suggest?conversation_id=${conversationId}`, {
          cache: "no-store",
        })
        const d = await r.json()
  ...
  const selectChat = async (id: string) => {
    ...
    await loadSuggestion(id, conv?.status)
  }
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (conv?.status !== "resolved") {
          await loadSuggestion(activeId, conv?.status)
        }
      }
    } catch {
    } finally {
  ...
  return (
    <main className="fixed inset-0 z-50 flex min-h-0 min-w-0 flex-col overflow-hidden bg-[#F7F5EF]">
      <div
        className="flex flex-none items-center justify-between px-4 py-2.5 shadow-sm"
        style={{
      ...
      <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
        <aside className="flex h-full min-h-0 w-80 flex-shrink-0 flex-col overflow-hidden border-r border-gray-200 bg-white">

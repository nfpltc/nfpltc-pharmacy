...
export async function POST(req: NextRequest) {
  try {
    ...
    // Prefer STATEMENT_FROM_EMAIL (e.g. noreply@) so statement emails can have a
    // different sender than contact-form emails. Fall back to FROM_EMAIL.
    const FROM_EMAIL     = process.env.STATEMENT_FROM_EMAIL || process.env.FROM_EMAIL
    ...
    // Sweep up any "queued" rows stuck for > 10 minutes. Likely from a previous
    // function timeout. These block duplicate-send protection, so we mark them
    // as failed and let this attempt proceed.
    const cutoff = new Date(Date.now() - 10 * 60 * 1000).toISOString()
    await sb.from("statement_email_log")
      .update({ status: "failed", error_message: "timeout recovery" })

...
export async function GET() {
  try {
    const { data, error } = await admin().from("contact_submissions").select("*").order("created_at", { ascending: false })
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "ID required" }, { status: 400 })
    const body = await req.json(); body.updated_at = new Date().toISOString()
    const { data, error } = await admin().from("contact_submissions").update(body).eq("id", id).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ submission: data })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}
